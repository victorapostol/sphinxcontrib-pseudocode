...
class Pseudocode(Directive):
    ...
    option_spec = {"linenos": directives.unchanged}
        ...
        if not pcode.strip():
            return [
                self.state_machine.reporter.warning(
                    'Ignoring "pcode" directive without content.', line=self.lineno
                )
            ]
        return pcode
...
def render_mm_html(
    self, node, code, options, prefix="pseudocode", imgcls=None, alt=None
):
    tag_template = """<pre id="{id}" style="display:hidden;">
            {code}
        </pre>"""
    self.body.append(
        tag_template.format(id=get_fignumber(self, node), code=self.encode(code))
    )
...
def write_katex_autorenderer_file(app, filename):
    filename = os.path.join(
        app.builder.srcdir, app._pseudocode_algorithm_static_path, filename
    )
...
def builder_inited(app):
    ...
    install_js(app)
    add_algorithm_katex_rendering(app)


def install_js(app, *args):
    # add required javascript
    app.add_js_file(
        f"https://cdn.jsdelivr.net/npm/pseudocode@latest/build/pseudocode.js"
    )
...
def add_algorithm_katex_rendering(app):
    filename_autorenderer = "katex_autorenderer_pseudocode_algorithm.js"
    write_katex_autorenderer_file(app, filename_autorenderer)
    app.add_js_file(filename_autorenderer)


def setup_static_path(app):
    app._pseudocode_algorithm_static_path = mkdtemp()
    if app._pseudocode_algorithm_static_path not in app.config.html_static_path:
        app.config.html_static_path.append(app._pseudocode_algorithm_static_path)


def builder_finished(app, exception):
    # Delete temporary dir used for _static file
    shutil.rmtree(app._pseudocode_algorithm_static_path)
